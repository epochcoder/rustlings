...
impl Package {
    fn new(sender_country: String, recipient_country: String, weight_in_grams: i32) -> Package {
        if weight_in_grams <= 0 {
            panic!("Cannot send an invinsible package!");
        } else {
            return Package {
                sender_country,
                recipient_country,
    ...
    fn get_fees(&self, cents_per_kg: i32) -> i64 {
        let package_kg = (self.weight_in_grams as f64) / 1000f64;
        let price = (package_kg * cents_per_kg as f64);
        println!("{}kg costs {} (cents_per_kg: {})", package_kg, price, cents_per_kg);
        price as i64
    }
}
